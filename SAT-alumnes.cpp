...
int getNextDecisionLiteral(){
	...
	int improvementThreshold = 5; 	// Quantes millores de seleccio de variables amb UNDEF abans de parar;
									// (Perque es visiten per ordre de major ocurrencies, pel que quant mes
									// s'avança menys probable es que es millori).
	while (i<ordOccurLit.size() and improvementCounter < improvementThreshold) {
		int var = ordOccurLit[i];
		if (currentValueInModel(var) == UNDEF and vsidsLit[var] > vsidsLit[nextDecVar]) {
				nextDecVar = var;
				improvementCounter++;
		}
		++i;
	...
	if (posLit[nextDecVar].size() >= negLit[nextDecVar].size())
		nextDecLit = nextDecVar;
	else
		nextDecLit = -nextDecVar;
